...
# ✅ الإضـافـة 3: قـواعـد بـيـانـات الـبـلاي لـيـسـت (Playlist)
...
# =============================================================
# ☁️ دوال الـمـسـتـودع (Warehouse Functions)
# =============================================================
...
async def skip_on(chat_id: int):
    skipmode[chat_id] = True
    user = await skipdb.find_one({"chat_id": chat_id})
    if user: return await skipdb.delete_one({"chat_id": chat_id})

async def skip_off(chat_id: int):
    skipmode[chat_id] = False
    user = await skipdb.find_one({"chat_id": chat_id})
    if not user: return await skipdb.insert_one({"chat_id": chat_id})
...
async def is_active_chat(chat_id: int) -> bool:
    if chat_id not in active: return False
    else: return True
...
async def is_active_video_chat(chat_id: int) -> bool:
    if chat_id not in activevideo: return False
    else: return True
...
async def is_maintenance():
    if not maintenance:
        ...
        if 1 in maintenance: return False
        else: return True
...
async def get_authuser(chat_id: int, name: str) -> Union[bool, dict]:
    ...
    if name in _notes: return _notes[name]
    else: return False
...
# =============================================================
# 🎵 دوال الـبـلاي لـيـسـت (Playlist Database)
# =============================================================
from config import SERVER_PLAYLIST_LIMIT

async def get_playlist_names(user_id: int) -> List[str]:
    names = []
    async for _playlist in playlistdb.find({"user_id": user_id}):
        names.append(_playlist["playlist"]["videoid"])
    return names

async def get_playlist(user_id: int, videoid: str) -> Union[Dict, None]:
    user = await playlistdb.find_one({"user_id": user_id, "playlist.videoid": videoid})
    if not user:
        return None
    return user["playlist"]

async def save_playlist(user_id: int, videoid: str, playlist: Dict):
    current = await get_playlist_names(user_id)
    if len(current) >= SERVER_PLAYLIST_LIMIT:
        return False
    await playlistdb.insert_one({"user_id": user_id, "playlist": playlist})
    return True

async def delete_playlist(user_id: int, videoid: str):
    try:
        await playlistdb.delete_one({"user_id": user_id, "playlist.videoid": videoid})
        return True
    except:
        return False
